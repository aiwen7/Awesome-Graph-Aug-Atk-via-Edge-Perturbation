...
from dgl import backend as F, batch, convert
from dgl.convert import from_networkx, graph as dgl_graph, to_networkx
...
class CitationGraphDataset(DGLBuiltinDataset):
    ...
    def process(self):
        ...
        """
        root = "./data" #self.raw_path
        objnames = ["x", "y", "tx", "ty", "allx", "ally"]
        graphnames = ["graph"]
        objects = []
        ...
        for i in range(len(graphnames)):
            with open(
                "{}/ind.{}.{}".format("/home/liuxin/PTDNet-Orig", self.name, graphnames[i]), "rb"
            ) as f:
                objects.append(_pickle_load(f))
        
        x, y, tx, ty, allx, ally, graph = tuple(objects)
        print("*******************")
        self.test = type(graph)
...
def main():
    ...
    graph_obj = data[0]
    #print(graph_obj)
    adj = data[0].adj_external(scipy_fmt="csr")
    #print(dgl.from_scipy(adj))
    print("data test")
    a = load_dict("ind.citeseer.graph")
    #print(type(a))
    print(a)
